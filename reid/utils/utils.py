...
## EDITED by: Nipun Nath (CSCE625: AI_Project_team_Group_17) ##
## Solves version compatibility issue ##
try:
...
def load_state_dict(model, src_state_dict):
  ...
  for name, param in src_state_dict.items():
    ...
    try:
      ## Works for python 2.7
      exec("try: dest_state_dict[name].copy_(param) \nexcept Exception, msg: print('Warning: Error occurs when copying '{}':}'.format(name, str(msg)))")
    except SyntaxError:
      ## Works for python 3.x
      try:
