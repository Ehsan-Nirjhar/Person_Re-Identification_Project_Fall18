...
class TripletLossAlignedReID(nn.Module):
    ...
    def forward(self, inputs, targets, local_features):
        ...
        #### Local Features for 8 horizontal and 4 vertical strips are separated for further calculation
        local_features_horz = local_features[:,0:8,:]
        local_features_vert = local_features[:,8:,:]
        
        
    
        p_local_features_horz = local_features_horz[p_inds]
        n_local_features_horz = local_features_horz[n_inds]
        local_dist_ap_horz = batch_local_dist(local_features_horz, p_local_features_horz)
        local_dist_an_horz = batch_local_dist(local_features_horz, n_local_features_horz)
        
        p_local_features_vert = local_features_vert[p_inds]
        n_local_features_vert = local_features_vert[n_inds]
        local_dist_ap_vert = batch_local_dist(local_features_vert, p_local_features_vert)
        local_dist_an_vert = batch_local_dist(local_features_vert, n_local_features_vert)

        # Compute ranking hinge loss
        y = torch.ones_like(dist_an)
        global_loss = self.ranking_loss(dist_an, dist_ap, y)
        local_loss_horz = self.ranking_loss_local(local_dist_an_horz ,local_dist_ap_horz , y)
        local_loss_vert = self.ranking_loss_local(local_dist_an_vert  ,local_dist_ap_vert , y)
        
        #### 2 types of local losses are combined
        local_loss = local_loss_horz + local_loss_vert
        if self.mutual:
